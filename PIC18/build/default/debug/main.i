...
double PWM_Stepper(double duty_ratio)
{
    ...
    CCP1_LoadDutyValue((uint16_t)((double)(duty_ratio)*4*(T2PR+1)));
    return duty_ratio;
}


double mapValue(uint16_t pot_val) {
    return 0.25 + (pot_val - 0) * (0.75 - 0.25) / (3800 - 0);
}
...
int main(void)
{
    ...
    while (1)
    {
        ...
        CCP1_LoadDutyValue((uint16_t)((double)(duty_cycle_ratio)*4*(T2PR+1)));
